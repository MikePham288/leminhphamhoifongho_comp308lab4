...
exports.trainAndPredictUsingUserParam = function (req, res) {
  // getting data from the api and casting to number
  const sepalLength = +req.body.sepallength;
  const sepalWidth = +req.body.sepalwidth;
  const petalLength = +req.body.petallength;
  const petalWidth = +req.body.petalwidth;
  ...
  //tensor of features for data from the api
  const testingData = tf.tensor2d([
  ...
  model.compile({
    loss: "meanSquaredError",
    //getting the learning rate from the api
  ...
  async function run() {
    const startTime = Date.now();
    // setting the number of epocs from the user entered value
    await model.fit(trainingData, outputData, {
      epochs: epoch,
      callbacks: {
        onEpochEnd: async (epo, log) => {
          lossValue = log.loss;
          console.log(`epoch ${epoch}; lossValue = ${log.loss}`);
          elapsedTime = Date.now() - startTime;
          console.log("elapsed time: " + elapsedTime);
